  ...
  return (
    <>
      <section className="container-wide pt-16 sm:pt-24 pb-16">
        ...
        <h1 className="display text-5xl sm:text-7xl lg:text-8xl font-light leading-[1] text-content max-w-5xl">
      ...
      {upcoming.length > 0 && (
        <section className="container-wide pb-24">
          <h2 className="display text-3xl text-content/85 mb-8">
            Coming up <span className="text-muted/60">({upcoming.length})</span>
          </h2>
      ...
      {past.length > 0 && (
        <section className="container-wide pb-32">
          <h2 className="display text-3xl text-content/85 mb-8">
            Past events <span className="text-muted/60">({past.length})</span>
          </h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
...
function EventCard({ ev, priority, dim }: { ev: any; priority?: boolean; dim?: boolean }) {
  ...
  return (
    <Link
      href={`/events/${ev.slug}`}
      className={`group flex flex-col rounded-2xl overflow-hidden bg-surface-2 card-hover ${dim ? 'opacity-90' : ''}`}
    >
      <div className="relative aspect-[5/6] bg-surface-3 overflow-hidden">
        {ev.image ? (
          <PayloadImage
            media={ev.image}
            size="feature"
        ...
        {date && (
          <div className="absolute top-4 left-4 bg-surface rounded-2xl px-4 py-2.5 flex items-center gap-3 shadow-sm">
            <div className="text-content leading-none">
              <div className="font-mono text-[0.6rem] uppercase tracking-[0.18em] text-muted/85">
            ...
            <div className="text-xs leading-tight">
              ...
              {ev.displayTime && (
                <div className="text-muted/85">{ev.displayTime}</div>
              )}
            </div>
          </div>
        )}
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        {ev.shortDescription && (
          <p className="mt-3 text-sm text-content/75 line-clamp-3">
            {ev.shortDescription}
          </p>
        )}
        <span className="mt-auto pt-5 inline-flex items-center gap-1 text-sm text-muted group-hover:gap-2 transition-all">
