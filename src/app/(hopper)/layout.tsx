...
export default async function HopperLayout({ children }: { children: React.ReactNode }) {
  const bar = await loadCafeBarData('hopper')

  return (
    <html lang="en" className={`${hopperDisplay.variable} ${hopperMono.variable}`}>
      <body className={`hopper-scope min-h-svh${bar.show ? ' has-hopper-bar' : ''}`}>
        {bar.show && (
          <CafeBar
            brand="hopper"
            bookUrl={bar.bookUrl}
