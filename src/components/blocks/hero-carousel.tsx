...
import type { CycleImage } from './hero-images'
 ...
 * the LCP element); the rest plus the controls mount after hydration and only
 * on non-reduced-motion, so reduced-motion users get a single static frame.
 *
 ...
 */
export function HeroCarousel({ images }: { images: CycleImage[] }) {
  // `order` maps a play position → an index into `images`. Identity on the
  // server / first render so it matches SSR; shuffled once `ready`.
  const [order, setOrder] = useState(() => Array.from({ length: images.length }, (_, i) => i))
  ...
  const [loaded, setLoaded] = useState<Set<string>>(() => new Set([images[0]?.image.src]))
  const markLoaded = (src: string) => setLoaded((s) => (s.has(src) ? s : new Set(s).add(src)))
  ...
  // hydration. Non-reduced-motion only.
  useEffect(() => {
  ...
  useEffect(() => {
    if (!ready) return
    const id = setTimeout(() => setFrame((f) => ({ step: f.step + 1, prev: f.step })), INTERVAL_MS)
      ...
      {ready && (
        <>
          ...
          <div className="absolute inset-x-0 bottom-4 z-30 flex justify-center drop-shadow-sm">
            <div className="relative h-5 overflow-hidden" style={{ width: maskWidth }}>
              <div
